...
TEST_F(BufferAssignmentTest, FastMergeManagerDirectTest) {
  ...
  TF_ASSERT_OK_AND_ASSIGN(auto large_color, buffer_large.color());
  ...
  TF_ASSERT_OK_AND_ASSIGN(
      bool reused, manager->TryReuseExistingAllocation(
...
TEST_F(BufferAssignmentTest,
       DefaultBufferAllocationsManagerWithDependencyOrdering) {
  auto builder = HloComputation::Builder(TestName());
  auto param = builder.AddInstruction(
      HloInstruction::CreateParameter(0, f32vec4_, "param"));
  auto add = builder.AddInstruction(
...
void BM_FastMergeManagerStress(::testing::benchmark::State& state) {
  ...
  for (auto s : state) {
    // 3. Benchmark Loop: Pause timing to reset tracked allocations in O(1)
    // time, then measure the exact sweep-line pooling and reuse performance.
    state.PauseTiming();
    ...
    for (int64_t i = 0; i < num_buffers; ++i) {
      ...
      TF_ASSERT_OK_AND_ASSIGN(auto color, buffer->color());
      if (i == 0 || !reuse_dist(generator)) {
        ...
        auto status_or = manager->TryReuseExistingAllocation(
...
BENCHMARK(BM_FastMergeManagerStress)->Range(1'000, 1'000'000);
