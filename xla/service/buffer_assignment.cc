...
class FastMergeBufferAllocationsManagerForComputationsWithoutOrdering
 ...
 public:
  ...
  // Registers an entry parameter or special allocation index.
  void RegisterSpecialAllocation(BufferAllocation::Index index) override {
    allocation_indices_.push_back(index);
  }
...
BufferAssigner::CreateAssignment(
  ...
  // 2. Primary Run: Execute buffer assignment using FAST_MERGE for sequential
  // computations, and the configured algorithm for computations without
  // ordering.
  absl::Status primary_status = RunAssignBuffers(
      module, global_computations, thread_local_computations, assignment.get(),
  ...
  if (absl::IsResourceExhausted(primary_status)) {
  ...
  } else if (primary_status.ok() && opts_.color_memory_limit) {
    // Aggregate total allocated bytes across all allocations by color.
    // Using absl::btree_map ensures deterministic iteration order by color.
    ...
    for (const BufferAllocation& alloc : assignment->Allocations()) {
      allocated_bytes_by_color[alloc.color()] += alloc.size();
    }

    for (const auto& [color, total_allocated_bytes] :
         allocated_bytes_by_color) {
      int64_t memory_limit = opts_.color_memory_limit(color);
      if (memory_limit > 0) {
  ...
  if (need_fallback) {
    ...
    RETURN_IF_ERROR(RunAssignBuffers(
        module, global_computations, thread_local_computations,
        assignment.get(), opts_.fallback_algorithm,
        buffer_assignment::
            AssignmentAlgorithmForComputationsWithoutOrderingProto::DEFAULT));
  }
...
absl::Status BufferAssigner::RunAssignBuffers(
  ...
  RETURN_IF_ERROR(
      AssignBuffersForComputations(thread_local_computations_no_fusion,
                                   /*is_thread_local=*/true,
                                   /*buffers_to_assign_sequentially=*/nullptr,
                                   assignment, non_seq_algorithm));
